...
async function generateFloorPlan(description, preferences = {}, forceHybrid = false) {
  if (!process.env.GEMINI_API_KEY || forceHybrid) {
    ...
    if (preferences.storeys) {
      const storeyStr = preferences.storeys.toLowerCase();
      if (storeyStr.includes("3")) floorsCount = 3;
      else if (storeyStr.includes("2")) floorsCount = 2;
      else if (storeyStr.includes("apartment")) isApartment = true;
    } else if (description) {
...
Ensure all room boundaries are logical (e.g. Master Bedroom is at least 4x4, Pooja Corner is smaller, bathrooms are next to bedrooms). No duplicate x/y offsets unless intended. Use positive dimensions.`;
  ...
  try {
    ...
    // Force ALL rooms generated by the 2D LLM to be on the Ground Floor (0)
    // The LLM cannot reliably generate 3D spatial coordinates, so we strip any hallucinated upper floors.
    parsed.rooms = parsed.rooms.filter(r => !r.floor || r.floor === 0);
    parsed.rooms.forEach(r => {
      r.floor = 0;
    });

    // Bulletproof procedural stacking for multi-storey
    if (floorsCount > 1) {
      // Inject staircase into ground floor if missing
      const hasStairs = parsed.rooms.some(r => r.name && r.name.toLowerCase().includes("stair"));
      if (!hasStairs) {
        parsed.rooms.push({
          id: `staircase_L0`,
          name: "Staircase",
          ...
          floor: 0,
          color: "#708090",
          furniture: ["Concrete Steps", "Steel Handrail"]
        });
      }

      const groundRooms = JSON.parse(JSON.stringify(parsed.rooms));

      // Generate upper floors by cloning the ground floor perfectly
      for (let f = 1; f < floorsCount; f++) {
        const upperRooms = JSON.parse(JSON.stringify(groundRooms));
        upperRooms.forEach(r => {
          r.floor = f;
          r.id = `${r.id}_L${f}`;
          if (r.name && !r.name.toLowerCase().includes("stair")) {
            r.name = `${r.name} (L${f})`;
          }
        });
